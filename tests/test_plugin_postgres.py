...
@pytest.mark.skipif(
    not pytest.config.getoption("--postgres"), reason="PostgreSQL testing not enabled"
)
class TestPluginPostgres:
        ...
        try:
            ...
            async with bot["plugins"]["pg"].connection() as pg_con:
                ...
                count = await pg_con.fetchval(
                    """SELECT count(*) FROM sirbot_test.hello"""
                )
        ...
        try:
            ...
            async with bot["plugins"]["pg"].connection() as pg_con:
                ...
                await pg_con.execute(
                    """INSERT INTO sirbot_test.foo (bar) VALUES ($1)""", "baz"
                )

            async with bot["plugins"]["pg"].connection() as pg_con:
                baz = await pg_con.fetchval(
                    """SELECT bar FROM sirbot_test.foo WHERE id = 1"""
                )
                name = await pg_con.fetchval(
                    """SELECT name FROM sirbot_test.hello WHERE id =1"""
                )
    ...
    async def test_no_migration_needed(self, bot, aiohttp_server):
        try:
            ...
            async with bot["plugins"]["pg"].connection() as pg_con:
                count_end = await pg_con.fetchval(
                    """SELECT count(*) FROM sirbot_test.hello"""
                )
